...
import { ChevronDown, House, Menu, Search, ShoppingBag, UserRound, X } from "lucide-react";
...
export function Header() {
  ...
  const isActive = (href: string) => {
    ...
    return (
      params.get("categoria") === new URLSearchParams(query).get("categoria")
    );
  };
  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-[#e5d8dc]/80 bg-[#fffdfb]/90 backdrop-blur-xl">
        <div className="page-shell grid h-[76px] grid-cols-[1fr_auto_1fr] items-center">
          ...
          >
            <Link
              href="/"
              className={`focus-ring grid size-8 place-items-center rounded-full hover:bg-[#f3e7e9] ${pathname === "/" ? "bg-[#f3e7e9] text-[#9e5f72]" : "text-[#9e5f72]"}`}
              aria-label="Inicio"
            >
              <House size={15} />
            </Link>
            {links.slice(0, 5).map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`focus-ring whitespace-nowrap rounded px-1.5 py-1 text-[11px] font-semibold hover:text-[#9e5f72] ${isActive(link.href) ? "bg-[#f3e7e9] text-[#9e5f72]" : ""}`}
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center justify-self-end gap-6">
            ...
            <div className="flex items-center gap-1">
              <Link
                href="/catalogo"
                className="focus-ring hidden rounded-full p-2.5 hover:bg-[#f3e7e9] sm:block"
                aria-label="Buscar"
              >
                <Search size={19} />
              </Link>
              {account ? (
                <div ref={accountMenuRef} className="relative hidden sm:block">
                  <button
                    ...
                    className="focus-ring inline-flex items-center gap-2 rounded-full px-2.5 py-2 hover:bg-[#f3e7e9]"
                ...
                <Link
                  href="/login"
                  className="focus-ring hidden items-center gap-2 rounded-full px-2.5 py-2 hover:bg-[#f3e7e9] sm:inline-flex"
                >
                  <UserRound size={19} />
                  <span className="text-[10px] font-bold">Ingresa</span>
                </Link>
              )}
              <button
                onClick={openCart}
                className="focus-ring relative rounded-full p-2.5 hover:bg-[#f3e7e9]"
      ...
      {mobileOpen && (
        <div
          className="fixed inset-0 z-50 bg-[#35282d]/25 backdrop-blur-sm"
          onClick={() => setMobileOpen(false)}
        >
          <div
            className="h-full w-[86%] max-w-sm bg-[#fffdfb] p-6"
            ...
            <nav className="mt-12 flex flex-col" aria-label="Menu movil">
              {links.slice(0, 5).map((link) => (
                <Link
                  ...
                  href={link.href}
                  onClick={() => setMobileOpen(false)}
                  className={`border-b border-[#e5d8dc] py-5 font-display text-2xl font-semibold ${isActive(link.href) ? "text-[#9e5f72]" : ""}`}
                >
                  {link.label}
                </Link>
              ))}
              <div className="border-b border-[#e5d8dc] py-5"><p className="font-display text-2xl font-semibold">Personalizaciones</p>{personalizedLinks.map((link) => <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className={`mt-3 block pl-4 text-sm font-bold ${isActive(link.href) ? "text-[#9e5f72]" : "text-[#786970]"}`}>{link.label}</Link>)}</div>
              {links.slice(5).map((link) => <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className={`border-b border-[#e5d8dc] py-5 font-display text-2xl font-semibold ${isActive(link.href) ? "text-[#9e5f72]" : ""}`}>{link.label}</Link>)}
              <Link
                href={account ? "/cuenta" : "/login"}
                onClick={() => setMobileOpen(false)}
                className="mt-8 rounded-full bg-[#35282d] px-5 py-3 text-center text-sm font-bold text-white"
              >
                {account ? "Mi cuenta" : "Ingresa"}
              </Link>
            </nav>
          </div>
        </div>
...
function AccountMenu({
  ...
  const customerLinks = [
    ...
    { href: "/cuenta/cotizaciones", label: "Chat de personalizados" },
  ];
  return (
    <div className="absolute right-0 top-12 w-56 rounded-2xl border border-[#e5d8dc] bg-[#fffdfb] p-2 shadow-xl">
      {customerLinks.map((item) => (
        <Link
          key={item.href}
          href={item.href}
          onClick={onClose}
          className="block rounded-xl px-3 py-2.5 text-xs font-bold hover:bg-[#f3e7e9]"
        >
          {item.label}
        </Link>
      ))}
      {isAdmin && (
        <Link
          href="/dashboard"
          onClick={onClose}
          className="mt-1 block rounded-xl bg-[#35282d] px-3 py-2.5 text-xs font-bold text-white"
        >
          Administración
        </Link>
      )}
      <button
        ...
        className="mt-1 w-full rounded-xl border-t border-[#e5d8dc] px-3 py-2.5 text-left text-xs font-bold text-[#9e5f72]"
      >
        Cerrar sesión
      </button>
    </div>
